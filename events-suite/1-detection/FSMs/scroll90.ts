// FSM «Scroll 90» — 1 vez por sesión: el usuario vio el 90% de la página, o
// sea que llegó al final. Sin config: el umbral ES la identidad de la máquina.

import { createFSM, DONE } from "./createFSM";
import { gateway } from "../../2-gateway";
import { scrollYData } from "../sources/scrollYData";
import { timeSession } from "../sources/timeSession";
import { BehaviorEventNames } from "../../types";

const LEVEL = 0.9;

export const startScroll90 = () =>
  createFSM<{ depth: number }, Record<string, never>>({
    id: "scroll90",
    initial: "watching",
    context: {},
    states: {
      watching({ depth }) {
        if (depth < LEVEL) return;
        gateway.emit(BehaviorEventNames.Scroll90, { engaged_seconds: timeSession.getSeconds() });
        return DONE;
      },
    },
    wire: send => [scrollYData.subscribe(gesture => send({ depth: gesture.scrollDepth }))],
  });
